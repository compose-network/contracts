...
contract ComposeAnchorStateRegistryTest is ComposeCommonTest {
    ...
    function test_initialization_success() public view {
        assertEq(
            address(composeAnchorStateRegistry.disputeGameFactory()),
            address(composeDisputeGameFactory),
            "DGF mismatch"
        );
        
        assertEq(
            address(composeAnchorStateRegistry.superchainConfig()),
            address(composeSuperchainConfig),
            "SuperchainConfig mismatch"
        );
        
        assertEq(
            composeAnchorStateRegistry.respectedGameType().raw(),
            5555,
