...
import { ComposeAnchorStateRegistry } from "@ssv/src/ComposeAnchorStateRegistry.sol";
import { ComposeDisputeGame } from "@ssv/src/ComposeDisputeGame.sol";
...
contract DeploySharedInfraInput is BaseDeployIO {
    ...
    function proofMaturityDelaySeconds() public view returns (uint256) {
        require(_proofMaturityDelaySeconds > 0, "DeploySharedInfraInput: proofMaturityDelaySeconds not set");
        return _proofMaturityDelaySeconds;
    }
    
    function disputeGameFinalityDelaySeconds() public view returns (uint256) {
        require(_disputeGameFinalityDelaySeconds > 0, "DeploySharedInfraInput: disputeGameFinalityDelaySeconds not set");
...
contract DeploySharedInfra is Script {
    ...
    /// @notice Load input from environment variables
    ...
    function deployBaseImplementations() internal {
        ...
        ComposeAnchorStateRegistry anchorStateRegistryImpl = new ComposeAnchorStateRegistry(
            input.disputeGameFinalityDelaySeconds()
        );
    ...
    function deployAndInitializeProxies() internal {
        ...
        Proposal memory placeholderProposal = Proposal({
    ...
    function deployDisputeGameImplementation() internal {
        ...
        ComposeDisputeGame disputeGameImpl = new ComposeDisputeGame(
            ...
            output.composeAnchorStateRegistryProxy(),
            input.authorizedProposer()
        );
    ...
    function assertValidInput() internal view {
        ...
        require(input.proofMaturityDelaySeconds() > 0, "Proof maturity delay not set");
        require(input.disputeGameFinalityDelaySeconds() > 0, "Dispute game finality delay not set");
    ...
    function assertValidOutput() internal view {
        ...
        IComposeAnchorStateRegistry asr = output.composeAnchorStateRegistryProxy();
        require(
            address(asr.disputeGameFactory()) == address(output.composeDisputeGameFactoryProxy()),
            "ASR DGF mismatch"
        );
        ...
        require(
            address(lockbox.superchainConfig()) == address(sc),
            "Lockbox SuperchainConfig mismatch"
        );
